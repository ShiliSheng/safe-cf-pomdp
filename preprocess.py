...
def preprocess_ETH(ETH_path, file): 
    ...
    file_path = "./test_data/ETH/"
    os.makedirs(file_path, exist_ok=True)
    raw_dataset.to_csv(file_path + "rawdata.csv")
...
def preprocess_SSD(SDD_path, scene_name, scene_video_id, scales_yaml_content):
    ...
    raw_dataset = reposition(raw_dataset)
    data_path = "./test_data/SDD-{}-{}/".format(scene_name, scene_video_id)
    os.makedirs(data_path, exist_ok=True)
    raw_dataset.to_csv(data_path + "rawdata.csv")
...
def create_online_test_dataset(df, test_data_file, min_cooldown, max_cooldown):
    new_df = pd.DataFrame()
    total_length = 3000
...
def create_training_dataset(train_raw_dataset, raw_dataset_path, data_name, history_length, prediction_length):
    ...
    train_data_file = data_name + "-history-{}-prediction-{}.pkl".format(history_length, prediction_length)
    train_data_file = os.path.join(raw_dataset_path, train_data_file)
    with open(train_data_file , 'wb') as handle:
        pickle.dump(train_dataset, handle, protocol=pickle.HIGHEST_PROTOCOL)

def prepare_train_validation_test(raw_dataset_path, training_length_settings):
    ...
    train_validation_id, test_id = train_test_split(raw_dataset.id.unique(), test_size= 1 / 10, random_state = 42)
    train_id, validation_id = train_test_split(train_validation_id, test_size = 0.2, random_state = 42)
    ...
    create_online_test_dataset(test_raw_dataset, raw_dataset_path, min_cooldown=5, max_cooldown=20)

    record = {"train_dataset": train_raw_dataset, "validation_dataset": validation_raw_dataset}
    for history_length, prediction_length in training_length_settings:
        for data_name, data in record.items():
            create_training_dataset(data, raw_dataset_path, data_name, history_length, prediction_length)
