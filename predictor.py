...
class TrainModel:
    def __init__(self, path, filename) -> None:
        ...
        self.path = './OpenTraj/datasets/ETH/seq_eth/'
    ...
    def preprocess(self, ):
        ...
        reshaped_dataset.to_csv( path + "test_dynammic_agents.csv")
    ...
    def train(self, LSTM_filename):
        with open(self.path + 'train_dataset.pickle', 'rb') as handle:
            train_dataset = pickle.load(handle)

        with open(self.path + 'test_dataset.pickle', 'rb') as handle:
        ...
        torch.save(model_metadata, LSTM_file)
        print("model saved")
...
class Predictor():
    ...
    def get_observations_from_moving_agents(self, dynamic_agents, cur_time):
        ...
        for i in range(2, len(dynamic_agents.columns), 2):
            values = dynamic_agents.loc[cur_time: cur_time + history_length, [dynamic_agents.columns[i], dynamic_agents.columns[i+1]]]
...
if __name__ == "__main__":
    ...
    raw_file = 'obsmat.txt'
    lstm_model = 'model_weights.pth'
    # d = TrainModel(path, raw_file)
    # d.preprocess()
    # d.train()

    pred = Predictor(path + lstm_model)
